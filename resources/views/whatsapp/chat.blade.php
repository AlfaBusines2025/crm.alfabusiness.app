                ...
                <div class="row" x-cloak x-data="{isContactListOpened:false,isContactCrmBlockOpened:false}">
                    ...
                    <div class="col-sm-12 col-md-3 col-lg-3 col-xl-3 mb-4 lw-contact-crm-block" :class="(!contact) ? 'lw-disabled-block-content' : ''" x-show="isContactCrmBlockOpened">
                            <div class="row">
								...
								<!-- Botón para abrir la lista de formularios -->
								<button type="button" 
										class="btn btn-primary float-right w-100" 
										data-toggle="modal" 
										data-target="#lwFormsModal">
									Forms
								</button>
     ...
     document.addEventListener('alpine:init', () => {
        Alpine.data('initialMessageData', () => ({
            ...
            allLabels: @json($allLabels),
		 	groups: @json($vendorContactGroups),
            ...
            labelsElement : function() {
             ...
             // Función de inicialización
                init() {
                    // Inicializar la variable global con el contacto actual (si existe)
                    window.selectedContact = this.contact ? { ...this.contact } : null;
                    // Observar cambios en 'contact' y actualizar la variable global
                    this.$watch('contact', (value) => {
                        if (value && Object.keys(value).length > 0) {
                            // Asignar los datos del contacto a la variable global
                            window.selectedContact = { ...value };
                            //console.log("Datos del Contacto Seleccionado Actualizado:", JSON.stringify(window.selectedContact, null, 2));
                        } else {
                            // Si no hay un contacto seleccionado, asignar null
                            window.selectedContact = null;
                            //console.log("No hay un contacto seleccionado.");
                        }
                    });
                }
        }));
        ...
        keyUp: function (editor, event) {
            if (event && event.which == 13 && !event.shiftKey && $.trim(this.getText())) { // On Enter
                ...
                appFuncs.resetForm();
								...
								<div class="modal fade" id="lwFormsModal" tabindex="-1" role="dialog" aria-labelledby="lwFormsModalLabel" aria-hidden="true">
								  <div class="modal-dialog" role="document"> <!-- SIN modal-dialog-scrollable -->
									<div class="modal-content">

									  <!-- Encabezado -->
									  <div class="modal-header">
										<h5 class="modal-title" id="lwFormsModalLabel">Formularios Disponibles</h5>
								...
								<div class="modal fade" id="lwFormEmbedModal" tabindex="-1" role="dialog" aria-labelledby="lwFormEmbedModalLabel" aria-hidden="true">
								  <div class="modal-dialog modal-dialog-centered modal-xl" style="max-width:90vw;" role="document">
									<div class="modal-content" style="height:90vh;">

									  <!-- Encabezado del modal -->
									  <div class="modal-header">
										<h5 class="modal-title" id="lwFormEmbedModalLabel">Formulario</h5>
										<button type="button" class="close" data-dismiss="modal" aria-label="Cerrar">
										  <span aria-hidden="true">&times;</span>
										</button>
									  </div>
								...
								<!-- Campo de Texto de WhatsApp (Asegúrate de que este campo exista en tu HTML) -->
								<!-- Este es el campo donde se insertará el enlace para WhatsApp -->
								<input type="text" id="whatsappTextInput" class="d-none"> 
<script>
  console.log(
    "Datos del Contacto Seleccionado Actualizado:",
    JSON.stringify(window.selectedContact, null, 2)
  );

  /**
   * En este caso, solo enviamos `_uid` y mapeamos `wa_id` a `phone_number`.
   * Si en un futuro deseas incluir más campos, los agregas aquí.
   */
  const CONTACT_FIELDS_TO_SEND = ["_uid", "wa_id"];

  /**
   * Extrae únicamente `_uid` y `wa_id` (renombrando `wa_id` como `phone_number`),
   * excluyendo valores null o undefined.
   *
   * @param {Object} contact - El objeto de contacto completo.
   * @returns {String} - La cadena de consulta serializada.
   */
  function extractAndSerializeContact(contact) {
    const params = {};

    // Aseguramos el _uid
    if (
      contact.hasOwnProperty("_uid") &&
      contact["_uid"] !== null &&
      contact["_uid"] !== undefined
    ) {
      params["_uid"] = contact["_uid"];
    }

    // Aseguramos el wa_id pero lo enviamos como phone_number
    if (
      contact.hasOwnProperty("wa_id") &&
      contact["wa_id"] !== null &&
      contact["wa_id"] !== undefined
    ) {
      params["phone_number"] = contact["wa_id"];
    }

    // Convertir el objeto params a una cadena de consulta
    const queryString = new URLSearchParams(params).toString();
    return queryString;
  }

  /**
   * Abre el modal #lwFormEmbedModal y ajusta el src del iframe con la URL del formulario,
   * incluyendo los datos específicos del contacto seleccionado como parámetros GET.
   * @param {String} formUrl - La URL base del formulario.
   * @param {String} formTitle - El título del formulario (opcional).
   */
  function openFormEmbedModal(formUrl, formTitle) {
    // Obtener los datos del contacto seleccionado desde la variable global
    const contact = window.selectedContact;

    // Verificar si hay un contacto seleccionado
    if (!contact) {
      console.error("No hay un contacto seleccionado.");
      alert("Por favor, selecciona un contacto antes de abrir el formulario.");
      return;
    }

    // Extraer y serializar los campos (solo _uid y phone_number)
    const queryString = extractAndSerializeContact(contact);

    // Construir la URL completa con los parámetros GET
    const urlWithParams = `${formUrl}?${queryString}`;

    // Asignar la URL al iframe
    const iframe = document.getElementById("lwFormEmbedIframe");
    if (iframe) {
      iframe.src = urlWithParams;
    } else {
      console.error("No se encontró el iframe con el ID 'lwFormEmbedIframe'.");
      return;
    }

    // Cambiar el título del modal dinámicamente (opcional)
    const modalLabel = document.getElementById("lwFormEmbedModalLabel");
    if (modalLabel) {
      modalLabel.innerText = formTitle || "Formulario";
    } else {
      console.warn(
        "No se encontró el elemento con el ID 'lwFormEmbedModalLabel' para actualizar el título."
      );
    }

    // Mostrar el modal usando jQuery
    $("#lwFormEmbedModal").modal("show");
  }
   ...
   * @param {String} formUrl - La URL base del formulario.
   */
  function copyFormLink(formUrl) {
    const contact = window.selectedContact;

    if (!contact) {
      console.error("No hay un contacto seleccionado.");
      alert(
        "Por favor, selecciona un contacto antes de copiar el enlace del formulario."
      );
      return;
    }
  ...
  /**
   * Envía el formulario por WhatsApp colocando el enlace en el campo de texto de WhatsApp.
   * Cierra los modales después de la acción.
   * @param {String} formUrl - La URL base del formulario.
   * @param {String} formName - El nombre del formulario.
   */
  function sendFormViaWhatsApp(formUrl, formName) {
    const contact = window.selectedContact;

    if (!contact) {
      console.error("No hay un contacto seleccionado.");
      alert(
        "Por favor, selecciona un contacto antes de enviar el formulario por WhatsApp."
      );
      return;
    }
    ...
    const message = `Hola ${
      contact.first_name || ""
    }, aquí tienes el formulario "${formName}": ${urlWithParams}`;

    // Asumiendo que el textarea tiene la clase 'lw-input-emoji'
    const emojioneareaInstance = $(".lw-input-emoji").data("emojioneArea");
    if (emojioneareaInstance) {
      emojioneareaInstance.setText(message);
    } else {
      console.error("No se encontró la instancia de emojioneArea.");
      alert("No se pudo insertar el enlace en el campo de mensaje.");
      return;
    }

    // Cerrar los modales
    $("#lwFormsModal").modal("hide");
    $("#lwFormEmbedModal").modal("hide");

    alert("El enlace del formulario ha sido colocado en el campo de mensaje. Puedes enviarlo.");
  }
</script>
@endpush
@endsection()
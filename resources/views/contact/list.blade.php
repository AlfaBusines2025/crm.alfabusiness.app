...
@php
    // Obtener el campo personalizado 'link_ubicacion_gps_negocio'
    $gpsCustomField = $vendorContactCustomFields->firstWhere('input_name', 'link_ubicacion_gps_negocio');

    // Extraer el '_id' del campo personalizado, si existe
    $gpsFieldId = $gpsCustomField ? $gpsCustomField->_id : null;
@endphp

@push('appScripts')
<script>
    // Pasar el ID del campo personalizado de GPS al JavaScript global
    window.gpsFieldId = "{{ $gpsFieldId }}";
</script>
@endpush
<div class="container-fluid mt-lg--6">
    <div class="row">
        ...
        <x-lw.modal id="lwDetailsContact" :header="__tr('Contact Details')">
            ...
            <script type="text/template" id="lwDetailsContactBody-template">
                ...
                <div>
                    ...
                    <div class="lw-details-item">
                        <%- __tData.wa_id %> 
						<a href="https://wa.me/<%- __tData.wa_id %>" class="btn btn-primary" target="_blank">
							<i class=" fab fa-whatsapp"></i>
						</a>
						<a href="tel:+<%- __tData.wa_id %>" class="btn btn-primary" target="_blank">
                ...
                <div>
                    ...
                    <div class="lw-details-item">
                        <%- __tData.email %> <a href="mailto:<%- __tData.email %>" class="btn btn-danger"><i class="fa fa-envelope"></i></a>
                    </div>
                </div>
				
				<fieldset>
					<legend>{{ __tr('Custom URLs') }}</legend>
					<% 
						// Inicializamos los contadores
						var filledCount = 0;
						var totalFields = {{ $vendorContactCustomFields->count() }};
					%>
					@foreach ($vendorContactCustomFields as $vendorContactCustomField)
						<% 
							// Obtiene el valor del campo personalizado correspondiente
							var fieldValue = _.get(
								_.find(__tData.custom_field_values, { 'contact_custom_fields__id': {{$vendorContactCustomField->_id}} }), 
								'field_value'
							); 

							// Asegurarse de que fieldValue sea una cadena antes de llamar a trim()
							var trimmedValue = (fieldValue || "").trim();

							// Verifica si el valor no está vacío, no es null y tiene más de un carácter
							var isFilled = trimmedValue.length > 1;

							// Si el campo está llenado, incrementamos el contador
							if (isFilled) {
								filledCount++;
							}

							// Verifica si el valor es una URL válida (comienza con http:// o https:// y contiene al menos un punto)
							var isURL = /^https?:\/\/.+\..+/.test(trimmedValue); 

							// Si es una URL válida, renderiza el botón
							if (isURL) {
						%>
							<div class="mb-2">
								<button 
								...
								>
									<i class="fa fa-external-link-alt"></i> {{ $vendorContactCustomField->input_name }}
								</button>
							</div>
						<% } %>
					@endforeach
					<% 
						...
						var percentage = totalFields > 0 ? ((filledCount / totalFields) * 100).toFixed(2) : 0;
					%>
					<div class="mt-3">
						<strong>Custom Contact Fields:</strong> <%= filledCount %> / <%= totalFields %> (<%= percentage %>%)
					</div>
				</fieldset>
                ...
                <fieldset>
					...
					@foreach ($vendorContactCustomFields as $vendorContactCustomField)
						<div class="mb-2">
							...
							<div class="lw-details-item">
								<% 
									var fieldValue = _.get(_.find(__tData.custom_field_values, {'contact_custom_fields__id' : {{$vendorContactCustomField->_id}} }), 'field_value'); 
									var isURL = /^https?:\/\/.+\..+/.test(fieldValue); // Verifica si el valor es una URL válida
								%>
								<% if (isURL) { %>
									<a href="<%- fieldValue %>" target="_blank" class="mr-2"><%- fieldValue %></a>
									<button type="button" class="btn btn-sm btn-link" onclick="window.open('<%- fieldValue %>', '_blank')">
        ...
        <div class="col-xl-12" x-cloak x-data="{isSelectedAll:false,selectedContacts: [],selectedGroupsForSelectedContacts:[],
            ...
            <x-lw.datatable data-page-length="100" id="lwContactList" :url="route('vendor.contact.read.list', [
                ...
                <th data-name="none" data-template="#lwSelectMultipleContactsCheckbox">{{ __tr('-') }}</th>
	...
	@php
		// IMPORTANTE!: Cambiar y permitir que luego pueda llenar el forms de cada uno
		$vendorUid = auth()->user()->vendors__id; // Asegúrate de que 'vendors__id' es el campo correcto
		$userId = auth()->user()->_id; //user id
		
	@endphp

	@if($vendorUid == 5 || $userId == 17)
		<a 
			title="{{ __tr('Forms') }}" 
			class="lw-btn btn btn-sm btn-default" 
			href="https://bocacorp.com/formulario-asistencia-marketing/?_uid=<%- __tData._uid %>"
			target="_blank"
			rel="noopener noreferrer">
			<i class="fa fa-form"></i> {{ __tr('Forms') }}
		</a>
	@endif

...
class AuthController extends BaseController
{
	...
	{
		// --- DEBUG 1: Ver todos los parámetros que llegan en el request ---
		// dump('DEBUG 1: Parámetros recibidos en la callback:');
		// dump($request->all());
		...
		// --- DEBUG 2: Verifica si el usuario denegó el acceso ---
		// dump('DEBUG 2: ¿Usuario denegó acceso? ', $userDenyRequest);

		// check the request is deny then redirect user on login page
		if (isset($userDenyRequest) && $userDenyRequest === 'access_denied') {
			// dump('DEBUG 2-B: Redireccionando a login por acceso denegado...');
		...
		try {
			// --- DEBUG 3: Forzamos la configuración de Google en tiempo de ejecución ---
			// dump('DEBUG 3: Configurando services.google en runtime...');
			// dump('REDIRECT URI', route('login.google.callback'));
			// dump('CLIENT ID', getAppSettings('google_client_id'));
			// dump('CLIENT SECRET', getAppSettings('google_client_secret'));
			...
			// --- DEBUG 4: Proceso de callback en tu AuthEngine ---
			// dump('DEBUG 4: Llamando a $this->authEngine->processCreateSocialCallBack()');
			$processReaction = $this->authEngine->processCreateSocialCallBack($provider);

			// dump('DEBUG 5: Resultado de $processReaction');
			// dump($processReaction); // Asumiendo que es un objeto con ->success(), ->message(), etc.

			// Verifica éxito
			if ($processReaction->success()) {
				// dump('DEBUG 6: Éxito. Redirigiendo a HOME...');
				return redirect()->route('home')->with([
					'message' => __tr('Welcome'),
				]);
			}

			// dump('DEBUG 7: Falla en processReaction, redirigiendo a login...');
			return redirect()->route('auth.login')->with([
				'error' => 'true',
			...
			// dump('DEBUG 8: Ocurrió una excepción:');
			// dump($e->getMessage());
			return redirect()->route('auth.login')->with([
				'error' => 'true',
				'message' => 'Ocurrió un error durante la autenticación con Google.',
			]);
			// throw $e;
		}
	}

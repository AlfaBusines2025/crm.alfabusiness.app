            ...
            if (!$aiBotReplyText and getVendorSettings('enable_open_ai_bot', null, null, $contact->vendors__id) and getVendorSettings('open_ai_access_key', null, null, $contact->vendors__id)) {
                try {
                    ...
                    if ($aiBotReplyText) {
						...
						$aiBotReplyText =  $botName ? ($aiBotReplyText) : $aiBotReplyText ;
    ...
    public function processWebhook($request, $vendorUid)
    {
        ...
        elseif ($messageObject) {
			...
			if (in_array($messageType, [
				...
				$latitude = Arr::get($messageObject, '0.location.latitude');
				$longitude = Arr::get($messageObject, '0.location.longitude');
				$locationName = Arr::get($messageObject, '0.location.name', 'Sin nombre');
				$locationAddress = Arr::get($messageObject, '0.location.address', 'Sin dirección');

				// Convertir a texto
				$messageBody = "*Ubicación Recibida*\n" .
								"URL Google Maps: https://maps.google.com/?q=$latitude,$longitude \n" .
							   "Nombre: $locationName\n" .
			...
			if (in_array($messageType, [
				...
				$contactsMessage = Arr::get($messageObject, '0.contacts', []);

				if (!empty($contactsMessage)) {
					// Inicializar el mensaje con un encabezado
					$messageBody = "👤 *Contactos Recibidos*\n\n";

					foreach ($contactsMessage as $index => $contactInfo) {
						// Obtener el nombre formateado si está disponible
						$formattedName = Arr::get($contactInfo, 'name.formatted_name', 'Sin nombre');
						
						// Obtener el nombre primero si está disponible
						$firstName = Arr::get($contactInfo, 'name.first_name', 'Sin primer nombre');
						
						// Obtener el nombre medio si está disponible
						$middleName = Arr::get($contactInfo, 'name.middle_name', 'Sin medio nombre');
						
						// Obtener el apellido primero si está disponible
						$lastName = Arr::get($contactInfo, 'name.last_name', 'Sin apellido');

						// Extraer emails y teléfonos
						$emails = Arr::get($contactInfo, 'emails', []);
						$phones = Arr::get($contactInfo, 'phones', []);

						// Construir la información del contacto
						$messageBody .= "🔹 *Contacto " . ($index + 1) . "*\n" .
										"Nombre Completo: " . $formattedName . "\n".
										"Primer Nombre: " . $firstName . "\n".
										"Segundo Nombre: " . $middleName . "\n".
										"Apellido: " . $lastName . "\n";

						// Manejar múltiples emails
						if (!empty($emails)) {
							foreach ($emails as $emailInfo) {
								$email = Arr::get($emailInfo, 'email', 'No disponible');
								$messageBody .= "📧 Email: " . $email . "\n";
							}
						} else {
							$messageBody .= "📧 Email: No disponible\n";
						}

						// Manejar múltiples teléfonos
						if (!empty($phones)) {
							foreach ($phones as $phoneInfo) {
								$phone = Arr::get($phoneInfo, 'phone', 'No disponible');
								$phoneType = Arr::get($phoneInfo, 'type', 'No disponible');
								$messageBody .= "📞 Teléfono: " . $phone . " (" . $phoneType . ")\n";
							}
						} else {
							$messageBody .= "📞 Teléfono: No disponible\n";
						}
					...
					// Depuración adicional: mostrar el contenido de messageObject
					error_log("Contacts not found. messageObject: " . print_r($messageObject, true));
					$messageBody = "📇 *Contacto recibido pero sin información disponible* " . json_encode($messageObject);
				}
               ...
               'interactive',
				...
				//si es imagen se almacena en message body:
				if (in_array($messageType, [
                'image', 'audio',
            	]))
				{
					$messageBody = $mediaData['caption']."url: ".$mediaData['link'];
				}

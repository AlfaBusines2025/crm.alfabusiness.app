...
class CustomApisService
{
  ...
  private function handleVendor_Default($params)
  {
	...
	// 1. Validación mínima de parámetros
				...
				//1.1.- EndPoint Datos de Web
				$web_endpoint = $general_endpoint."web".$general_parameters_endpoint;
				$web_url = $web_endpoint;
		
				//1.2.- EndPoint Datos de KeyWords General
				$keywords_general_endpoint = $general_endpoint."keywords/general".$general_parameters_endpoint;
		
				//1.2.1.- EndPoint Datos de KeyWords Commerce
				$keywords_commercel_endpoint = $general_endpoint."keywords/commerce".$general_parameters_endpoint;
		
				//1.2.2.- EndPoint Datos de KeyWords Web
				$keywords_commercel_endpoint = $general_endpoint."keywords/web".$general_parameters_endpoint;
				
				//1.3.- Endpoint Interacciones de usuarios
				$interacciones_usuario_endpoint = $general_endpoint."user".$general_parameters_endpoint;
				$interaccionesUrl = $interacciones_usuario_endpoint;
				
				//1.4.- a estos productos se el agrega al final un keywords para que se pueda traer los datos
		
				//1.4.1.- Endpoint de Búsqueda de productos
				$products_search_endpoint = $general_endpoint."search/products".$general_parameters_endpoint;
				
				//1.4.2.- Endpoint de Búsqueda de páginas
				$pages_search_endpoint = $general_endpoint."search/pages".$general_parameters_endpoint;
		
				//1.4.3.- Endpoint de Búsqueda General
				$general_search_endpoint = $general_endpoint."search/general".$general_parameters_endpoint;
		
				//1.5.- Endpoint de Búsqueda de Ubicaciones
				...
				return response()->json([
					  ...
					  'web'                    => "Los datos de la página web son: " . $web_data,
					  'vendor_id' => $params['vendor_id'],
					  'message' => 'Proveedor no tiene una función personalizada. Respuesta por defecto.'
					], 200);	
	}
  ...
  private function callOpenAi($apiKey, $idOrg, $payload)
  {
    try {
      ...
      if ($response->failed()) {
        \Log::error("handleVendor_10 OpenAI Error. Code: {$response->status()}. Response: {$response->body()}");
        return [
          'error'   => true,
  ...
  private function handleVendor_11($params)
  {
    ...
    if (isset($dataProductos['productos_similares']) && count($dataProductos['productos_similares']) > 0) {
      ...
      $productos_filtrados = array_filter($productos_respuesta, function ($producto) {
        return (isset($producto['imagen_producto']) && strlen($producto['imagen_producto']) > 10) && ($producto['similarity'] > 0.30);
      });
      ...
      $productos_enviados = array_slice($productos_filtrados, 0, 3);

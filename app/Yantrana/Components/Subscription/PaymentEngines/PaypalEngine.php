...
class PaypalEngine extends BaseEngine
{
    ...
    public function paypalCaptureOrder($inputData)
    {
		...
		if(isset($inputData['suscriptionID'])){
			
			$subscriptionID = $inputData['suscriptionID'];
			$accessToken = $this->generateAccessToken();

			// Llamamos al endpoint de PayPal para obtener los detalles de la suscripción
			$response = Http::withHeaders([
				'Content-Type'  => 'application/json',
				'Authorization' => "Bearer $accessToken",
			])->get("{$this->paypalCheckoutUrl}/v1/billing/subscriptions/{$subscriptionID}");

			$subscriptionData = json_decode($response->getBody()->getContents());

			// Se pueden considerar estados como 'ACTIVE' o 'APPROVED' según lo que necesites
			if (!__isEmpty($subscriptionData) && in_array($subscriptionData->status, ['ACTIVE', 'APPROVED'])) {
				return $this->engineSuccessResponse([
					'capturedPaypalData' => $subscriptionData,
					'txn_reference'            => $subscriptionData->id,
					'manual_subscription_uid'  => $inputData['manualSubscriptionUid'],
				], __tr('Subscription activated successfully'));
			} else {
				return $this->engineFailedResponse(['show_message' => true], __tr('Subscription not active or failed'));
			}

...
class ContactEngine extends BaseEngine implements ContactEngineInterface
{
    ...
    public function processContactCreate($inputData) 
	{
		if (isExternalApiRequest()) {
			return $this->createContactApi($inputData);
		} else {
			return $this->createContactWeb($inputData);
		}
	}
	
	protected function createContactApi($inputData)
	{
		$vendorId = getVendorId();
		...
		try {
			// Verificar límite de contactos (plan)
			$currentContactCount = $this->contactRepository->countIt(['vendors__id' => $vendorId]);
			$vendorPlanDetails = vendorPlanDetails('contacts', $currentContactCount, $vendorId);

			if (!$vendorPlanDetails['is_limit_available']) {
				// Límite alcanzado, abortar y devolver respuesta
				DB::rollBack();
				return $this->engineResponse(22, null, $vendorPlanDetails['message']);
			}

			// Crear el contacto en la BD
			if (!$contactCreated = $this->contactRepository->storeContact($inputData)) {
				// Falló la creación del contacto
				DB::rollBack();
				return $this->engineFailedResponse(
					['debug' => ['inputData' => $inputData]],
					__tr('Contact not added.')
				);
			}

			// Asignar Grupos basados en títulos
			...
			if (!empty($contactGroupsTitles)) {
				...
				if (!empty($groupsToBeCreatedTitles)) {
					foreach ($groupsToBeCreatedTitles as $groupTitle) {
						if (strlen($groupTitle) > 255) {
							abortIf(strlen($groupTitle) > 255, null, __tr('Group title should not be greater than 255 characters'));
						}
						$groupsToBeCreated[] = [
							'title' => $groupTitle,
							'vendors__id' => $vendorId,
			...
			// Asignar Campos Personalizados basados en `input_name`
			$customInputFields = $inputData['custom_input_fields'] ?? [];
			$customInputFieldUidsAndValues = [];

			if (!empty($customInputFields)) {
				$customInputFieldsFromDb = $this->contactCustomFieldRepository->fetchItAll(array_keys($customInputFields), [], 'input_name')->keyBy('input_name');

				foreach ($customInputFields as $inputName => $fieldValue) {
					$customFieldFromDb = $customInputFieldsFromDb[$inputName] ?? null;

					if (!$customFieldFromDb || $customFieldFromDb->vendors__id != $vendorId) {
						continue;
					}

					$customInputFieldUidsAndValues[] = [
						'contact_custom_fields__id' => $customFieldFromDb->_id,
						'contacts__id' => $contactCreated->_id,
			...
			], __tr('Contact created via API'));
		} catch (\Exception $e) {
			// Rollback de la transacción en caso de error
			DB::rollBack();

			// Registrar el error en los logs
			Log::error('Error al crear contacto vía API:', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);

			// Devolver una respuesta de error
			return $this->engineFailedResponse(
				['debug' => ['error' => $e->getMessage()]],
				__tr('Failed to create contact via API.')
			);
		}
	}
	
	protected function createContactWeb($inputData)
	{
        $vendorId = getVendorId();
	...
	{
		if (isExternalApiRequest()) {
			return $this->updateContactApi($contactIdOrUid, $inputData);
		} else {
			return $this->updateContactWeb($contactIdOrUid, $inputData);
		}
	}
	
	protected function updateContactApi($contactIdOrUid, $inputData) 
	{
		$vendorId = getVendorId();
		...
		try {
			...
			$contactWhereClause = [
				'vendors__id' => $vendorId,
				'wa_id' => $contactIdOrUid,
			];

			// Obtener el contacto con sus grupos
			...
			$updateData = [
				'whatsapp_opt_out' => (isset($inputData['whatsapp_opt_out']) && $inputData['whatsapp_opt_out']) ? 1 : null,
				'disable_ai_bot'    => (isset($inputData['enable_ai_bot']) && $inputData['enable_ai_bot']) ? 0 : 1,
			];

			// Actualizar campos básicos si existen en el input
			$basicFields = ['first_name', 'last_name', 'language_code', 'email', 'country'];
			foreach ($basicFields as $field) {
				if (array_key_exists($field, $inputData)) {
					if ($field === 'country') {
						$updateData['countries__id'] = findRequestedCountryId($inputData['country']);
					} else {
			...
			if (!$this->contactRepository->updateIt($contact, $updateData)) {
				DB::rollBack();
				return $this->engineFailedResponse([], __tr('Failed to update contact'));//.contact:'.json_encode($contact)." . updateData: ".json_encode($updateData)));
			}
			*/
			...
			if (!empty($customInputFields)) {
				...
				foreach ($customInputFields as $inputName => $fieldValue) {
					...
					// Si el valor es null o está vacío, se omite y no se crea el valor
					if (is_null($fieldValue) || $fieldValue === null || $fieldValue === "null" || $fieldValue === '' || $fieldValue === "https://forms.alfabusiness.app/storage/app/" || $fieldValue === "['']" || $fieldValue === '[""]') {
						continue;
					}
					...
					$existingCustomFieldValue = ContactCustomFieldValueModel::where('contact_custom_fields__id', $customFieldFromDb->_id)
					...
					if ($existingCustomFieldValue) {
						...
						$existingCustomFieldValue->field_value = $fieldValue;
						$existingCustomFieldValue->save();
					} else {
			...
			return $this->engineSuccessResponse(
				...
				__tr('Contact details updated via API.')
			);
		} catch (\Exception $e) {
			// Rollback de la transacción en caso de error
			DB::rollBack();
			...
			// Registrar el error en los logs
			Log::error('Error al actualizar contacto vía API:', ['error' => $e->getMessage(), 'trace' => $e->getTraceAsString()]);

			// Devolver una respuesta de error
			return $this->engineFailedResponse(
				['debug' => ['error' => $e->getMessage()]],
				__tr('Failed to update contact via API.')
			);
		}
	}
	
	protected function updateContactWeb($contactIdOrUid, $inputData) 
	{
			$vendorId = getVendorId();
			$contactWhereClause = [
				'vendors__id' => $vendorId,
	...
	/**
	 * Obtener un contacto por su _uid y su wa_id (phoneNumber),
	 * usando el repositorio y cargando relaciones.
	 *
	 * @param  string  $contactUid
	 * @param  int     $vendorId
	 * @return mixed   Contact model or null
	 */
	public function getContactByUidAndWaId($contactUid, $vendorId)
	{
		// Preparar el array de búsqueda
		$contactWhereClause = [
			'vendors__id' => $vendorId,
			'_uid'        => $contactUid,
		];

		// Devuelve el contacto con el resto de relaciones,
		// pero en 'customFieldValues' solo el último (ordenado desc y limit(1))

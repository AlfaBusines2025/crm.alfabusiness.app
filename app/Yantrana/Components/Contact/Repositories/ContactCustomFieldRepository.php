...
class ContactCustomFieldRepository extends BaseRepository implements ContactCustomFieldRepositoryInterface
{
	...
	public function fetchLastValidCustomFieldValue($customFieldId, $contactId)
	{
		return ContactCustomFieldValueModel::where('contact_custom_fields__id', $customFieldId)
			->where('contacts__id', $contactId)
			->whereNotNull('field_value')
			->where('field_value', '!=', '')
			->orderBy('updated_at', 'desc')  // <-- Orden descendente por fecha de creación
			->first();
	}
}
